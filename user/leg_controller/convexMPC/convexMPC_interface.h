...
struct update_data_Fex_t
{
  float p[3];
  float v[3];
  float q[4];
  float w[3]; 
  float Fex[3];               //带外力MPC专用
  float r[12]; 
  float yaw;
  float weights[18];          //带外力MPC专用
  float traj[12*K_MAX_GAIT_SEGMENTS];
  float alpha;
  unsigned char gait[K_MAX_GAIT_SEGMENTS];
  unsigned char hack_pad[1000];
...
EXTERNC void update_Fex_problem_data_floats(float* p, float* v, float* q, float* w, float* Fex,
